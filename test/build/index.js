...
// src/compile.ts
function mapToCalls(schema) {
  const { properties } = schema;
  const calls = {};
  const fnImports = /* @__PURE__ */ new Set();
  let argFn = 0;
  const argFns = /* @__PURE__ */ new Map();
  function fnNameWithArgs(name, args) {
  ...
  function addCall(propertyCalls, property, fn, ...args) {
    fnImports.add(fn);
    let usableName = fn;
    if (args.length) {
...
var generateDefineFn = ({ argFnName, args, name }) => `
...
`.trim();
function compileJsonSchemaToSource(schema, { helperPath } = {
...
__validations__
  ...
  `.trim().replace("__imports__", fnImports.join(",\n")).replace("__fn_defs__", argFns.map(generateDefineFn).join("\n")).replace("__validations__", Object.entries(calls).map(([property, calls2]) => `validate(obj, '${property}', [${calls2.join(", ")}]),`).join("\n"));
  return out;
}
...
async function testFixture(inputFile) {
  const outputFile = `test/fixtures/${(0, import_path.basename)(inputFile)}.js`;
  const output = compileJsonSchemaToSource(JSON.parse(await (0, import_promises.readFile)(inputFile, "utf8")));
  if (!await exists(outputFile)) {
    await (0, import_promises.writeFile)(outputFile, output);
  }
}
async function test() {
  const inputFiles = await (0, import_fast_glob.default)("test/fixtures/*.json");
